...
const sortedCampyears = computed(() => {
  return campyears.value.sort((a: Campyear, b: Campyear) => a.year - b.year);
});

apiClient.get("/stafplicatie/v1/campyear").then((response) => {
...
function toggleOpen(campyear: any) {
  if (campyear.open) {
    apiClient
      .post(`/stafplicatie/v1/campyear/${campyear.year}/close`)
      .then((response) => {
...
function makeActive(campyear: any) {
  apiClient
    ...
    .then((response) => {
      ...
      campyear.active = true;
    });
}

function deleteCampyear(index: number) {
  const campyear = campyears.value[index];
  apiClient
    .delete(`/stafplicatie/v1/campyear/${campyear.year}`)
    .then((response) => {
      campyears.value.splice(index, 1);
    })
    .catch((error) => {
      console.log(error);
    });
  ...
  >
          ...
          <tr v-for="(campyear, index) in sortedCampyears" :key="campyear.year">
            ...
            <td class="py-5 px-4">
              <p class="text-black dark:text-white">
                {{ formatTime(campyear.start) }}
              </p>
            </td>
            <td class="py-5 px-4">
              <p class="text-black dark:text-white">
                {{ formatTime(campyear.end) }}
              </p>
            </td>
            <td class="py-5 px-4">
              <div>
                ...
                >
                  <div class="relative">
                    <input
                      ...
                      :id="`active-${index}`"
                      class="sr-only"
                      @change="makeActive(campyear)"
                      :disabled="campyear.active"
                    />
                    <div
                      :class="{
                        '!bg-success': campyear.active,
                      }"
                      class="block h-8 w-14 rounded-full bg-danger"
                    ></div>
                    <div
                      :class="campyear.active && '!right-1 !translate-x-full'"
                ...
                >
                  <div class="relative">
                    <input
                      ...
                      :id="`open-${index}`"
                      class="sr-only"
                      @change="toggleOpen(campyear)"
                      :disabled="!campyear.active"
                    />
                    <div
                      :class="{
                        '!bg-success': campyear.open,
                        'opacity-50': !campyear.active,
                      }"
                      class="block h-8 w-14 rounded-full bg-danger"
                    ></div>
                    <div
                      :class="campyear.open && '!right-1 !translate-x-full'"
                      class="absolute left-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white transition"
                    ></div>
                  </div>
                </label>
	...
	S61.9,13.7,61.2,13z M20.6,52.1c-0.1,0.1-0.2,0.1-0.3,0.2L7.4,56.6l4.3-12.9c0-0.1,0.1-0.2,0.2-0.3L39.4,16l8.7,8.7L20.6,52.1z
	 M51.2,21.5l-8.7-8.7l6.1-6.1c2.9,2.8,5.8,5.8,8.6,8.7L51.2,21.5z"
                ...
                <!-- <button
                  v-if="!campyear.active"
                  @click="deleteCampyear(index)"
